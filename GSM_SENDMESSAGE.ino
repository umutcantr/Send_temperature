...
void setup() {
 ...
 void loop(){
   ...
   Gsm_Sendmsg(message);
   PORTB=0x04;   //Turn on blue led (to understand the sms is gone)
   delay(2000);
 ...
 void Gsm_init(){
  ...
  PORTB=0x02;             // Open red led (to understand the gsm module is working)
  delay(1000);
}
 ...
 unsigned int ADC_read()
{
 ...
 void Gsm_Sendmsg(char *message){
   ...
   delay(1000);
   mySerial.println("AT+CMGS=\"90XXXXXXXXXX\"\r");  //Send telephone number
   delay(500);
